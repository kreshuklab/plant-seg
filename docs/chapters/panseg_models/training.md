...
!!! warning
    The recommended way to train models is through the [napari training GUI](../panseg_interactive_napari/unet_training.md).
...
You can import models trained using this library into PanSeg by choosing
`ADD CUSTOM MODEL` in the model selection drop-down in the segmentation tab.
Models are by default stored in `~/.panseg_models/`.
