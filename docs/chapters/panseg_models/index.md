...
We publicly release the datasets used for training the networks which available as part of the *PanSeg* package.
...
The following pre-trained networks are provided with PanSeg package out-of-the box and can be specified in the config file or chosen in the GUI.
...
Selecting a given network name (either in the config file or GUI) will download the network into the `~/.panseg_models` directory.
Detailed description of network training can be found in our [paper](https://doi.org/10.7554/eLife.57613).

The PanSeg home directory can be configured with the `PANSEG_HOME` environment variable.

```bash
export PANSEG_HOME="/path/to/panseg/home"
```
