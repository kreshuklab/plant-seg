...
def test_dt_watershed_and_clustering(shape, layout, stacked, is_nuclei, clustering, mode):
    ...
    result_clustering = clustering_segmentation_task(image=image, over_segmentation=result, mode=mode)
    assert result_clustering.semantic_type == SemanticType.SEGMENTATION
    assert result_clustering.image_layout == property.image_layout
    assert result_clustering.voxel_size == property.voxel_size
...
def test_mutex():
    ...
    result_clustering = clustering_segmentation_task(image=image, over_segmentation=None, mode='mutex_ws')
    assert result_clustering.semantic_type == SemanticType.SEGMENTATION
    assert result_clustering.image_layout == property.image_layout
    assert result_clustering.voxel_size == property.voxel_size
    assert result_clustering.shape == mock_data.shape
