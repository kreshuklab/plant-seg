...
def get_preprocessing_workflow():
    ...
    container = MainWindow(widgets=[widget_gaussian_smoothing,
                           ...
                           labels=False)
    container = setup_menu(
        container, path='https://kreshuklab.github.io/plant-seg/chapters/plantseg_interactive_napari/data_processing.html')
    return container
...
def get_gasp_workflow():
    container = MainWindow(widgets=[widget_unet_predictions,
                           ...
                           labels=False)
    container = setup_menu(
        container, path='https://kreshuklab.github.io/plant-seg/chapters/plantseg_interactive_napari/unet_gasp_workflow.html')
    return container
...
def get_extra():
    ...
    widget_fix_over_under_segmentation_from_nuclei.quantile.native.setStyleSheet(
        STYLE_SLIDER)
    container = MainWindow(widgets=[widget_extra_manager,
                                    Label(),
...
def get_proofreading():
    container = MainWindow(widgets=[widget_split_and_merge_from_scribbles,
