...
def widget_unet_predictions(
    ...
    layer_type = 'image'
    step_kwargs = dict(
        model_name=model_name if mode == PREDICTION_MODE_P else None,
    ...
    return start_prediction_process(
        unet_predictions_wrapper,
...
def _compute_multiple_predictions(image, patch_size, patch_halo, device, use_custom_models=True):
    ...
    for i, model_name in enumerate(model_list):
        napari_formatted_logging(
            f'Running UNet Predictions: {model_name} {i}/{len(model_list)}', thread='UNet Grid Predictions'
        )
...
@magicgui(
    call_button='Try all Available Models',
    image={'label': 'Image', 'tooltip': 'Raw image to be processed with a neural network.'},
    patch_size={'label': 'Patch size', 'tooltip': 'Patch size use to processed the data.'},
    patch_halo={
        'label': 'Patch halo',
    ...
    device={'label': 'Device', 'choices': ALL_DEVICES},
    use_custom_models={'label': 'Use custom models', 'tooltip': 'If True, custom models will also be used.'},
)
def widget_test_all_unet_predictions(
    image: Image,
    patch_size: tuple[int, int, int] = (80, 170, 170),
...
def _compute_iterative_predictions(
    pmap, model_name, num_iterations, sigma, patch_size, patch_halo, single_batch_mode, device
):
    func = partial(
        unet_predictions,
        model_name=model_name,
...
@magicgui(
    call_button='Run Iterative Predictions',
    image={'label': 'Image', 'tooltip': 'Raw image to be processed with a neural network.'},
    model_name={
        'label': 'Select model',
    ...
    num_iterations={'label': 'Num. of iterations', 'tooltip': 'Number of iterations the model will run.'},
    sigma={
        'label': 'Sigma',
...
def widget_iterative_unet_predictions(
    image: Image,
    model_name: str,
    ...
    return start_threading_process(
        _compute_iterative_predictions,
