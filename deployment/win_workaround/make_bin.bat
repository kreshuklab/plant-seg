...
echo y | call conda create -n %version_gpu% -c pytorch -c conda-forge -c lcerrone python=3.9 plantseg
call conda pack -n %version_gpu% -o %version_gpu%-win64.zip

ECHO Creating CPU env
set version_cpu=plant-seg-cpu
echo y | call conda create -n %version_cpu% -c pytorch -c conda-forge -c lcerrone python=3.9 plantseg cpuonly
